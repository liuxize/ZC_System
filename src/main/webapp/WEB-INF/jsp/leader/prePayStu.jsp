                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/leader/prePayStu?page=1&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">首页</a></li>
                                <c:if test="${pagingVO.curentPageNo <= 1}">
                                    <li><a href="/leader/prePayStu?page=1&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">&laquo;上一页</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo > 1}">
                                    <li><a href="/leader/prePayStu?page=${pagingVO.upPageNo}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">&laquo;上一页</a></li>
                                </c:if>

                                <li class="active"><a href="">${pagingVO.curentPageNo}</a></li>
                                <c:if test="${pagingVO.curentPageNo+1 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/leader/prePayStu?page=${pagingVO.curentPageNo+1}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">${pagingVO.curentPageNo+1}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+2 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/leader/prePayStu?page=${pagingVO.curentPageNo+2}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">${pagingVO.curentPageNo+2}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+3 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/leader/prePayStu?page=${pagingVO.curentPageNo+3}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">${pagingVO.curentPageNo+3}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+4 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/leader/prePayStu?page=${pagingVO.curentPageNo+4}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">${pagingVO.curentPageNo+4}</a>
                                    </li>
                                </c:if>

                                <c:if test="${pagingVO.curentPageNo <pagingVO.totalCount}">
                                    <li><a href="/leader/prePayStu?page=${pagingVO.nextPageNo}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">下一页&raquo;</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo >=pagingVO.totalCount}">
                                    <li><a href="/leader/prePayStu?page=${pagingVO.totalCount}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">下一页&raquo;</a></li>
                                </c:if>
                                <li><a href="/leader/prePayStu?page=${pagingVO.totalCount}&gradeid=${gradeIndex}&subjectid=${subjectIndex}&typeid=${typeIndex}&campusid=${campusIndex}">尾页</a></li>
...
<script type="text/javascript">
    ...
    function jumpPage(){
        ...
        if(page<=${pagingVO.totalCount}){
            window.location.href="/leader/prePayStu?page=" + page+ "&gradeid=" + ${gradeIndex}+ "&subjectid=" + ${subjectIndex}+ "&typeid=" + ${typeIndex} +"&campusid=" + ${campusIndex};
        }
    }
