                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <button class="btn btn-success col-md-1" style="margin-top: 20px; float:right"
                                onClick="location.href='/master/addNoteTable?dicid=${dicid}&currentpage=${pagingVO.curentPageNo}'">
                            添加信息
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button type="button" class="btn btn-info  btn-xs"
                                        onclick="location.href='/master/modifyNoteTable?noteID=${item.noteid}&dicid=${item.dicid}&page=${pagingVO.curentPageNo}'">修改
                                </button>
                            </td>
                        </tr>
                    </c:forEach>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/master/editNoteTable?dicid=${dicid}&page=1">首页</a></li>
                                <c:if test="${pagingVO.curentPageNo <= 1}">
                                    <li><a href="/master/editNoteTable?dicid=${dicid}&page=1">&laquo;上一页</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo > 1}">
                                    <li><a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.upPageNo}">&laquo;上一页</a></li>
                                </c:if>

                                <li class="active"><a href="">${pagingVO.curentPageNo}</a></li>
                                <c:if test="${pagingVO.curentPageNo+1 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.curentPageNo+1}">${pagingVO.curentPageNo+1}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+2 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.curentPageNo+2}">${pagingVO.curentPageNo+2}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+3 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.curentPageNo+3}">${pagingVO.curentPageNo+3}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+4 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.curentPageNo+4}">${pagingVO.curentPageNo+4}</a>
                                    </li>
                                </c:if>

                                <c:if test="${pagingVO.curentPageNo <pagingVO.totalCount}">
                                    <li><a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.nextPageNo}">下一页&raquo;</a></li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo >=pagingVO.totalCount}">
                                    <li><a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.totalCount}">下一页&raquo;</a></li>
                                </c:if>
                                <li><a href="/master/editNoteTable?dicid=${dicid}&page=${pagingVO.totalCount}">尾页</a></li>
...
<script type="text/javascript">
    ...
    function jumpPage(){
        ...
        if(page<=${pagingVO.totalCount}){
            window.location.href="/master/editNoteTable?dicid="+ ${dicid}+ "&page=" + page;
        }
    }
    ...
    function remove(noteID,dicID,page) {
        var msg = "您确定要删除吗";
        if (confirm(msg) == true) {
            window.location.href="/master/removeNoteTable?noteID=" + noteID + "&dicID="+ dicID + "&currentPage=" +page;
            return true;
        } else {
            return false;
        }
